
import turtle


def border(t, screen_x, screen_y):

    # Lift the pen and move the turtle to the center.
    t.penup()
    t.home()

    # Move to lower left corner of the screen
    t.forward(screen_x / 3)
    t.right(90)
    t.forward(screen_y / 3)
    t.setheading(180)  # t.right(90) would also work.

    # Draw the border
    t.pencolor('black')
    t.pendown()
    t.pensize(5)
    for distance in (screen_x, screen_y, screen_x, screen_y):
        t.forward(distance)
        t.right(90)

    # Raise the pen and move the turtle home again;
    t.penup()
    t.home()


def mrec(t, size, color):
    """(Turtle, int, str)

    Draw a rectangular shape of the chosen colour and size.
    """
    t.pencolor(color)
    t.pendown()
    for i in range(2):
        t.forward(size + 1)
        t.right(90)
        t.backward(50)
        t.right(90)
        t.forward(200)
        t.left(90)
        t.backward(50)
        t.left(90)
        t.forward(200)
        t.left(90)


def main():
    # Create screen and turtle.
    screen = turtle.Screen()
    screen.title('mrec Demo')
    screen_x, screen_y = screen.screensize()
    t = turtle.Turtle()

    # Uncomment to draw the graphics as quickly as possible.
    ##t.speed(0)

    # Draw a border around the canvas
    border(t, screen_x, screen_y)

    # Draw a set of nested shapes, varying the color.
    # The shapes are 10%, 20%, etc. of half the size of the canvas.
    colors = ['red', 'orange', 'yellow', 'green', 'blue', 'violet']
    t.pensize(3)
    for i, color in enumerate(colors):
        mrec(t, (screen_y / 3) / 10 * (i + 2), color)

    print('Hit any key to exit')
    dummy = input()


if __name__ == '__main__':
    main()